...
from faceweave.vision import detect_fps
...
def extract_frames(target_path : str, fps : float) -> bool:
	temp_frame_compression = round(31 - (faceweave.globals.temp_frame_quality * 0.31))
...
def merge_video(target_path : str, fps : float) -> bool:
	...
	temp_frames_pattern = get_temp_frames_pattern(target_path, '%04d')
	commands = [ '-hwaccel', 'auto', '-r', str(fps), '-i', temp_frames_pattern, '-c:v', faceweave.globals.output_video_encoder ]
