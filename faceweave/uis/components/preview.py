...
from faceweave.typing import Frame, Face, FaceSet
from faceweave.vision import get_video_frame, count_video_frame_total, normalize_frame_color, resize_frame_dimension, read_static_image, read_static_images
...
from faceweave.uis.typing import ComponentName
...
def render() -> None:
        ...
        if is_image(faceweave.globals.target_path):
                target_frame = read_static_image(faceweave.globals.target_path)
        ...
        if is_video(faceweave.globals.target_path):
                temp_frame = get_video_frame(faceweave.globals.target_path, faceweave.globals.reference_frame_number)
...
def update_preview_image(frame_number : int = 0) -> gradio.Image:
        ...
        source_frames = read_static_images(faceweave.globals.source_paths)
        source_face = get_average_face(source_frames)
        reference_face = get_reference_faces() if 'reference' in faceweave.globals.face_selector_mode else None
        if is_image(faceweave.globals.target_path):
                target_frame = read_static_image(faceweave.globals.target_path)
        ...
        if is_video(faceweave.globals.target_path):
                temp_frame = get_video_frame(faceweave.globals.target_path, frame_number)
