...
from faceweave.core import limit_resources, conditional_process
from faceweave.uis.core import get_ui_component
from faceweave.normalizer import normalize_output_path
from faceweave.filesystem import is_image, is_video, clear_temp
...
def start(output_path : str) -> Tuple[gradio.Image, gradio.Video]:
	faceweave.globals.output_path = normalize_output_path(faceweave.globals.source_paths, faceweave.globals.target_path, output_path)
	limit_resources()
	conditional_process()
	if is_image(faceweave.globals.output_path):
		return gradio.Image(value = faceweave.globals.output_path, visible = True), gradio.Video(value = None, visible = False)
	if is_video(faceweave.globals.output_path):
		return gradio.Image(value = None, visible = False), gradio.Video(value = faceweave.globals.output_path, visible = True)
