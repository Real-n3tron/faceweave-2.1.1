...
from faceweave.face_helper import warp_face, paste_back
from faceweave.face_store import get_reference_faces
from faceweave.content_analyser import clear_content_analyser
from faceweave.typing import Face, FaceSet, Frame, Update_Process, ProcessMode, ModelSet, OptionsWithModel, Embedding
from faceweave.filesystem import is_file, is_image, are_images, is_video, resolve_relative_path
...
def get_frame_processor() -> Any:
	...
	with THREAD_LOCK:
		if FRAME_PROCESSOR is None:
			...
			FRAME_PROCESSOR = onnxruntime.InferenceSession(model_path, providers = faceweave.globals.execution_providers)
...
def pre_process(mode : ProcessMode) -> bool:
	...
	if not faceweave.globals.skip_download and not is_download_done(model_url, model_path):
		...
		return False
	if not are_images(faceweave.globals.source_paths):
		logger.error(wording.get('select_image_source') + wording.get('exclamation_mark'), NAME)
		return False
	for source_frame in read_static_images(faceweave.globals.source_paths):
	...
	if mode in [ 'output', 'preview' ] and not is_image(faceweave.globals.target_path) and not is_video(faceweave.globals.target_path):
		...
		return False
	if mode == 'output' and not faceweave.globals.output_path:
...
def swap_face(source_face : Face, target_face : Face, temp_frame : Frame) -> Frame:
	...
	if 'box' in faceweave.globals.face_mask_types:
		crop_mask_list.append(create_static_box_mask(crop_frame.shape[:2][::-1], faceweave.globals.face_mask_blur, faceweave.globals.face_mask_padding))
	...
	if 'region' in faceweave.globals.face_mask_types:
		crop_mask_list.append(create_region_mask(crop_frame, faceweave.globals.face_mask_regions))
...
def prepare_source_frame(source_face : Face) -> Frame:
	source_frame = read_static_image(faceweave.globals.source_paths[0])
...
def process_frame(source_face : Face, reference_faces : FaceSet, temp_frame : Frame) -> Frame:
	if 'reference' in faceweave.globals.face_selector_mode:
		similar_faces = find_similar_faces(temp_frame, reference_faces, faceweave.globals.reference_face_distance)
		if similar_faces:
			for similar_face in similar_faces:
				temp_frame = swap_face(source_face, similar_face, temp_frame)
	if 'one' in faceweave.globals.face_selector_mode:
		target_face = get_one_face(temp_frame)
		if target_face:
			temp_frame = swap_face(source_face, target_face, temp_frame)
	if 'many' in faceweave.globals.face_selector_mode:
