...
from faceweave.typing import Process_Frames
from faceweave.execution_helper import encode_execution_providers
...
def multi_process_frames(source_paths : List[str], temp_frame_paths : List[str], process_frames : Process_Frames) -> None:
	with tqdm(total = len(temp_frame_paths), desc = wording.get('processing'), unit = 'frame', ascii = ' =', disable = faceweave.globals.log_level in [ 'warn', 'error' ]) as progress:
		...
		with ThreadPoolExecutor(max_workers = faceweave.globals.execution_thread_count) as executor:
			...
			queue_per_future = max(len(temp_frame_paths) // faceweave.globals.execution_thread_count * faceweave.globals.execution_queue_count, 1)
