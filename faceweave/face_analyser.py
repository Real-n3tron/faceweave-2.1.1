...
from faceweave.face_helper import warp_face, create_static_anchors, distance_to_kps, distance_to_bbox, apply_nms
from faceweave.filesystem import resolve_relative_path
from faceweave.typing import Frame, Face, FaceSet, FaceAnalyserOrder, FaceAnalyserAge, FaceAnalyserGender, ModelSet, Bbox, Kps, Score, Embedding
from faceweave.vision import resize_frame_dimension
...
def get_face_analyser() -> Any:
	...
	with THREAD_LOCK:
		if FACE_ANALYSER is None:
			if faceweave.globals.face_detector_model == 'retinaface':
				face_detector = onnxruntime.InferenceSession(MODELS.get('face_detector_retinaface').get('path'), providers = faceweave.globals.execution_providers)
			if faceweave.globals.face_detector_model == 'yunet':
			...
			if faceweave.globals.face_recognizer_model == 'arcface_blendswap':
				face_recognizer = onnxruntime.InferenceSession(MODELS.get('face_recognizer_arcface_blendswap').get('path'), providers = faceweave.globals.execution_providers)
			if faceweave.globals.face_recognizer_model == 'arcface_inswapper':
				face_recognizer = onnxruntime.InferenceSession(MODELS.get('face_recognizer_arcface_inswapper').get('path'), providers = faceweave.globals.execution_providers)
			if faceweave.globals.face_recognizer_model == 'arcface_simswap':
				face_recognizer = onnxruntime.InferenceSession(MODELS.get('face_recognizer_arcface_simswap').get('path'), providers = faceweave.globals.execution_providers)
			gender_age = onnxruntime.InferenceSession(MODELS.get('gender_age').get('path'), providers = faceweave.globals.execution_providers)
			FACE_ANALYSER =\
			{
...
def extract_faces(frame: Frame) -> List[Face]:
	face_detector_width, face_detector_height = map(int, faceweave.globals.face_detector_size.split('x'))
	...
	elif faceweave.globals.face_detector_model == 'yunet':
